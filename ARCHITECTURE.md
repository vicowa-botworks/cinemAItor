...
- `migrations/`: Ordered, idempotent SQL migrations tracked in `schema_migrations`, applied by
  ...
  transaction, since the pragma is a no-op inside one) — this is what lets table-rebuild
  migrations (e.g. `0031` rebuilding `assets`) run without cascading `ON DELETE CASCADE` wipes
